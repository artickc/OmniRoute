 ...
 *
 * Only providers whose upscale API is implemented here are listed:
 ...
 * configured Stability AI / Topaz Labs account works with no extra setup.
 *
 * Adding a provider is two steps: a row here plus a handler branch in
 * `handlers/imageUpscale.ts` keyed on `format`.
 */
...
export interface UpscaleProviderConfig {
  ...
  authHeader: string;
  format: "stability-upscale" | "topaz-upscale";
